...
yellow "一、更换下二级域名自定义名称再尝试执行脚本（重要）"
green "例：原二级域名 x.ygkkk.eu.org 或 x.ygkkk.cf ，在cloudflare中重命名其中的x名称"
echo
yellow "二：因为同个本地IP连续多次申请证书有时间限制，等一段时间再尝试" && exit
fi
}
...
if [[ -n $caacme1 && ! $caacme1 == "Main_Domain" ]] && [[ -f /root/cert.crt && -f /root/private.key && -s /root/cert.crt && -s /root/private.key ]]; then
caacme=$caacme1
else
caacme='无证书申请记录'
fi
...
yellow "四、泛域名申请前须要在解析平上设置一个名称为 * 字符的解析记录 (输入格式：*.一级/二级主域)"
yellow "公钥文件crt保存路径：/root/cert.crt"
yellow "密钥文件key保存路径：/root/private.key"
echo
red "========================================================================="
acmeshow
