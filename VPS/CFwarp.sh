...
latestV=$(curl -s https://gitlab.com/rwkgyg/CFwarp/-/raw/main/version/version | awk -F "更新内容" '{print $1}' | head -n 1)
...
[[ "$result1" == *"page-404"* ]] && NF="遗憾，当前IP仅解锁Netflix自制剧" || NF="恭喜，当前IP完整解锁Netflix非自制剧"
}
...
lncf
curl -s https://gitlab.com/rwkgyg/CFwarp/-/raw/main/version/version | awk -F "更新内容" '{print $1}' | head -n 1 > /root/warpip/v
...
wget -N https://gitlab.com/rwkgyg/cfwarp/raw/main/wgcf_2.2.20_$cpu -O /usr/local/bin/wgcf && chmod +x /usr/local/bin/wgcf         
...
yellow " 是否安装WARP？"
yellow " 目前的两大优点："
yellow " 一、有机率完整解锁 Netflix 与 ChatGPT"
yellow " 二、可选择接管本地IPV4、IPV6、Socks5的出站IP"
echo "-------------------------------------------------------------------"
green " 1. 选择 warp-go 方案进入WARP安装菜单(推荐)"
green " 2. 选择 wgcf    方案进入WARP安装菜单"
green " 0. 退出脚本"
white "=================================================================="
echo
readp " 请输入数字【0-2】:" Input
case "$Input" in
 1 ) ONEWARPGO;;
 2 ) ONEWGCFWARP;;
 * ) exit
